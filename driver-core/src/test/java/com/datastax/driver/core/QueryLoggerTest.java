...
import static com.datastax.driver.core.QueryLogger.UNKNOWN_STATEMENT;
 ...
 * More tests specifically targeting unsuccessful queries can be found in
...
public class QueryLoggerTest extends CCMBridge.PerClassSingleNodeCluster {
    ...
    @Test(groups = "unit")
    public void should_not_log_null_statements() throws Exception {
        // given
        normal.setLevel(DEBUG);
        // when
        queryLogger = QueryLogger.builder().build();
        queryLogger.update(null, null, null, 0);
        // then
    ...
    public void should_not_log_special_statements() throws Exception {
        ...
        // when
        queryLogger = QueryLogger.builder().build();
        queryLogger.update(null, Statement.DEFAULT, null, 0);
        // then
        String line = normalAppender.get();
        assertThat(line).isEmpty();
    }
    ...
    public void should_log_slow_queries() throws Exception {
        ...
        slow.setLevel(DEBUG);
        queryLogger = QueryLogger.builder()
            .withSlowQueryLatencyThresholdMillis(0)
            .build();
        cluster.register(queryLogger);
        // when
        String query = "SELECT * FROM test where pk = ?";
