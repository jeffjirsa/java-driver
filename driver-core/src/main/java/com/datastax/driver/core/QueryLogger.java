...
public class QueryLogger implements LatencyTracker {
    ...
    @VisibleForTesting static final String UNKNOWN_STATEMENT = "??Unknown Statement??";
    ...
    public void update(Host host, Statement statement, Exception exception, long newLatencyNanos) {
        // do not log null statements nor special statements
        if (statement == null || statement == Statement.DEFAULT) {
            return;
        }
        Logger logger;
        String template;
        long latencyMs = MILLISECONDS.convert(newLatencyNanos, NANOSECONDS);
        if (exception == null) {
